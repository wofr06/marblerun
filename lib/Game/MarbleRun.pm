...
EOF
	...
	my $elems = [
		...
		yM=>'Vertical Cannon',
		'k+'=>'Sky Height Tile Small', 'k2'=>'Sky Height Tile Large',
		kA=>'Sky Launcher', kZ=>'Sky Landing', kC=>'Sky Curve',
		kY=>'Sky Curve2', kX=>'Sky Junction', kD=>'Sky Drop',
		kG=>'Sky Catcher curved', kI=>'Sky Catcher Straight', kW=>'Sky 3 in 1',
		kH=>'Sky Catcher minus', kJ=>'Sky Catcher plus',
		kK=>'Sky Curve long minus', kL=>'Sky Curve long plus',
		kM=>'Sky Curve medium minus', kN=>'Sky Curve medium plus',
		kQ=>'Sky Curve short minus', kR=>'Sky Curve short plus',
		...
		kk=>'Sky Rail long minus', kl=>'Sky Rail long plus',
		km=>'Sky Rail medium minus', kn=>'Sky Rail medium plus',
		ks=>'Sky Rail short minus', kt=>'Sky Rail short plus',
		kv=>'Sky connection',
		# Self made elements
	...
	my $sets = [
		...
		'Starter Set Skytrax', 46, ['_'=>4, o=>3, 1=>8, C=>7, X=>1, H=>1, K=>1,
			l=>3, m=>6, s=>9, 'k+'=>8, k2=>8, kv=>24, kk=>1, kl=>1, km=>1,
			kn=>1, ks=>1, kt=>1, kA=>1, kC=>3, kY=>3, kX=>2, kP=>2, kD=>1,
			kH=>1, kJ=>1, kK=>1, kL=>1, kW=>1, kZ=>1, kM=>1, kN=>1, kQ=>2,
			kR=>2, kS=>2, kT=>2, kU=>1, kV=>1, kG=>1, kI=>1],
		# Extension Sets
		...
		'Multiform', 40, ['*'=>14, xS=>1, 1=>10, '+'=>2, C=>6, X=>1, Z=>1, o=>1,
			K=>1, l=>2, m=>4, s=>4],
		'Game Splitter', 47, ['@'=>3, C=>6, W=>1, X=>1, A=>1, Z=>1, '+'=>4,
			1=>11, yS=>1, l=>2, m=>2, s=>5, c=>1, d=>1, o=>2],
		'Game Switch', 48, ['@'=>4, C=>7, xS=>1, S=>3, Z=>2, 1=>12, W=>1, X=>1,
			l=>1, m=>2, s=>4, o=>1],
		# 2023
		'Releaser', 37, [yR => 1],
		...
		for my $pass (1, 2) {
			...
			for my $r (@$rail) {
				...
				if ($sym =~ /^x[sml]/) {
					...
					my ($t1) = grep {$_->[0] == $r->[2]} @$tile;
					my ($t2) = grep {$_->[0] == $r->[4]} @$tile;
					my $tile_1 = loc($self->{elem_name}{$t1->[2]});
					my $tile_2= loc($self->{elem_name}{$t2->[2]});
					say loc("From %1 (%2) to %3 (%4) %5 %6", $pos1, $tile_1, $pos2, $tile_2, $name,
